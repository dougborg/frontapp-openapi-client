...
if TYPE_CHECKING:
    from ._base import FrontappPydanticBase

# Registries mapping class names and types
_attrs_to_pydantic: dict[type, type[FrontappPydanticBase]] = {}
_pydantic_to_attrs: dict[type[FrontappPydanticBase], type] = {}
_attrs_name_to_class: dict[str, type] = {}
_pydantic_name_to_class: dict[str, type[FrontappPydanticBase]] = {}


def register(attrs_class: type, pydantic_class: type[FrontappPydanticBase]) -> None:
    ...
    Raises:
        TypeError: If attrs_class is not an attrs class or pydantic_class is not
            a subclass of FrontappPydanticBase.
        ValueError: If the classes are already registered with different mappings.
    """
    # Import FrontappPydanticBase at runtime to avoid circular imports
    from ._base import FrontappPydanticBase as BaseClass
    ...
    if not isinstance(pydantic_class, type) or not issubclass(
        ...
        msg = f"{pydantic_class.__name__} is not a subclass of FrontappPydanticBase"
        raise TypeError(msg)
...
def get_pydantic_class(attrs_class: type) -> type[FrontappPydanticBase] | None:
...
def get_attrs_class(pydantic_class: type[FrontappPydanticBase]) -> type | None:
...
def get_pydantic_class_by_name(name: str) -> type[FrontappPydanticBase] | None:
