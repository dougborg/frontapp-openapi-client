"""Base class for Frontapp Pydantic models with attrs conversion support.
...
class FrontappPydanticBase(BaseModel):
    ...
    Example:
        ```python
        from frontapp_public_api_client.models import Product as AttrsProduct
        from frontapp_public_api_client.models_pydantic import (
        ...
        for field_name, field_value in self.model_dump().items():
            ...
            if isinstance(converted_value, dict):
                ...
                if nested_pydantic_class and issubclass(
                    nested_pydantic_class, FrontappPydanticBase
                ):
...
def _convert_to_attrs_value(
    ...
    if isinstance(value, FrontappPydanticBase):
...
def _get_field_type(
    model_class: type[FrontappPydanticBase], field_name: str
) -> type | None:
