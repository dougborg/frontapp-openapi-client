...
This guide explains the testing architecture and approach for the Frontapp OpenAPI
Client project.
...
- **External consistency**: Validation against official Frontapp documentation
...
- Validates against comprehensive documentation from developer.frontapp.com
...
**`test_frontapp_client.py`** - Custom client implementation and transport layer\
...
- **Real API tests**: Require `FRONTAPP_API_KEY` in `.env` file
...
| `frontapp_client.py` | 85.3%    | ✅ Good      | Maintain, improve to 90%+ |
...
uv run pytest --cov=frontapp_public_api_client --cov-report=json -m 'not docs'
uv run poe analyze-coverage

# Generate HTML report for detailed inspection
uv run pytest --cov=frontapp_public_api_client --cov-report=html -m 'not docs'
...
See [Issue #31](https://github.com/dougborg/frontapp-openapi-client/issues/31) for
detailed improvement plan targeting:

- `frontapp_client.py`: 85% → 90%+ (add edge case tests)
- `client.py`: 51% → 70%+ (test common patterns)
- `log_setup.py`: 0% → 60%+ (add basic smoke tests)

## External Dependencies

- **Official Frontapp documentation**: Tests validate against
  `developer.frontapp.com` content
- **Generated client validation**: Ensures openapi-python-client generates valid code
- **Real API integration**: Optional tests with actual Frontapp API (credentials
  required)
