...
Example from `api/conversations/list_conversations.py`:

```python
# Synchronous
def sync_detailed(...) -> Response[ListConversationsResponse200]:
    """List conversations (sync)."""
    ...

def sync(...) -> ListConversationsResponse200 | None:
    """List conversations (sync, parsed only)."""
    ...

# Asynchronous
async def asyncio_detailed(...) -> Response[ListConversationsResponse200]:
    """List conversations (async)."""
    ...

async def asyncio(...) -> ListConversationsResponse200 | None:
    """List conversations (async, parsed only)."""
    ...
```
...
async def main():
    async with FrontappClient() as client:
        ...
        convs = response.parsed.field_results
...
async def async_wrapper():
    ...
    return await loop.run_in_executor(
        None,
        lambda: list_conversations.sync_detailed(client=client)
    )
```
...
async with FrontappClient() as client:
    convs_task = list_conversations.asyncio_detailed(client=client)
    contacts_task = list_contacts.asyncio_detailed(client=client)

    convs, contacts = await asyncio.gather(convs_task, contacts_task)
```
...
with FrontappClient() as client:
    response = list_conversations.sync_detailed(client=client)
    for conv in response.parsed.field_results:
        print(conv.subject)
```
...
`FrontappClient` provides both sync and async transports:
