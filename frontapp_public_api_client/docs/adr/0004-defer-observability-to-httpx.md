...
requests_total = Counter('frontapp_requests', 'Total requests')
...
The one observability feature the client **does** include is sensitive-data redaction in
structured logs — `Authorization` headers and any field matching common secret patterns
(`api_key`, `password`, `token`, etc.) are scrubbed before hitting the logger. See
`_logging.py` for the filter.
...
```python
from frontapp_public_api_client.plugins import PrometheusPlugin, OTelPlugin
...
async with FrontappClient() as client:
    ...
    convs = await client.conversations.list(limit=10)
```
...
requests_total = Counter('frontapp_requests_total', 'Total requests', ['method', 'endpoint'])
request_duration = Histogram('frontapp_request_duration_seconds', 'Request duration')
