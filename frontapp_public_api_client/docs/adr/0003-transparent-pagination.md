...
The Frontapp API uses cursor-based pagination with Link headers:
...
1. Fetch first page (default limit: 250, Frontapp's max for efficiency)
...
```python
client = FrontappClient(max_pagination_pages=200)
```
...
- [AutoPaginationTransport Implementation](../../frontapp_public_api_client/frontapp_client.py)
- [Frontapp API Pagination Documentation](https://help.frontapp.com/api)
