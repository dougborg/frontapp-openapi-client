"""Utility functions for working with Frontapp API responses.
...
# Front's OpenAPI spec doesn't define a single canonical error schema — each
# endpoint models its own error responses inline. We dispatch errors by HTTP
# status and store whatever attrs/dict payload the generator produced in
# ``error_response`` as ``Any``.
...
class ValidationError(APIError):
    ...
    field name to the list of human-readable error messages that Frontapp
...
def unwrap[T](response: Response[T], *, raise_on_error: bool = True) -> T | None:
    ...
    if not (200 <= status < 300):
        ...
        error_message = (
            ...
            else f"Front API returned status {status}"
        )

        if status == HTTPStatus.UNAUTHORIZED:
            raise AuthenticationError(error_message, status, parsed)
        if status == HTTPStatus.UNPROCESSABLE_ENTITY:
            raise ValidationError(error_message, status, parsed)
...
def unwrap_data[T, DataT](
    ...
    *,
    ...
    For Frontapp's wrapped list endpoints like ``GET /orders`` which return
...
def get_error_message[T](response: Response[T]) -> str | None:
    ...
    if response.parsed is None or 200 <= response.status_code < 300:
        return None

    message_raw = getattr(response.parsed, "message", None)
