...
class Resource:
    """Thin async CRUD wrapper around a single Frontapp API resource.
    ...
    def _load_module(self, func_name: str) -> ModuleType:
        ...
        ``frontapp_public_api_client.api.<module>.<func_name>``.
        """
        if func_name not in self._module_cache:
            ...
            path = f"frontapp_public_api_client.api.{self._config.module}.{func_name}"
