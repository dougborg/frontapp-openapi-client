...
- **Wrapping API methods for retries** — Resilience (retries, rate limiting, pagination)
  is at the transport layer. All 7 endpoints get it automatically via `FrontappClient`.
...
- **Pagination: page + per_page (not page + limit)** — Frontapp uses `per_page` (max
...
- `frontapp_public_api_client/` — Python client with transport-layer resilience
- `frontapp_mcp_server/` — MCP server for AI assistants (9 tools)
- `packages/frontapp-client/` — TypeScript client
