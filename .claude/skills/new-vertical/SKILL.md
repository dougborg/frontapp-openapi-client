...
allowed-tools:
  ...
  - Bash(uv run poe full-check)
  - Bash(uv run poe fix)
  - Bash(uv run poe facts)
...
1. **Read the facts file.** Open `docs/api-facts.yaml` and find the resource
...
2. **Plan.** Invoke the `vertical-planner` agent with the issue # or resource
...
3. **Domain projection.** Create
   ...
   `domain/__init__.py`.
4. **Helper class.** Create `frontapp_public_api_client/helpers/<resource>.py`
   ...
   - Unwrap based on `list_shape` from the facts file:
...
5. **Wire into FrontappClient.** In `frontapp_client.py`:
...
10. **Tests.** Add helper tests at `tests/test_<resource>.py` (mock httpx
...
11. **README.** Flip the API Coverage row from ⏳ to ✅.

12. **Validate.** `uv run poe full-check`. This includes `facts-check`, so
