...
2. **Review the YAML diff** before regenerating. This is the load-bearing step.
   ```bash
   git diff docs/frontapp-openapi.yaml
   ```
   (Pipe through `less` if the diff is large and your terminal supports it.)
...
4. **Regenerate the client.** Do not cancel.
   ...
   ```
   Or, equivalently, `uv run poe regenerate-all` runs vendor-spec → regenerate
   → facts in one shot.

5. **Regenerate the API facts index.**
   ```bash
   uv run poe facts
   ```
   This rewrites `docs/api-facts.yaml` from the new generated tree. Skipping
   this step will leave the agent-facing fact index stale; CI's
   `facts-check` will catch it and fail. **Always commit `docs/api-facts.yaml`
   alongside the regenerated client** — they are a single logical change.

6. **Run agent-check to surface new errors.**
   ```bash
...
7. **Fix at the source.** No `noqa`, no `type: ignore`, no skipped tests. If
   the upstream change is genuinely incompatible, raise it with the user
   before working around it.

8. **Commit generated files separately from hand-written fixes.** Two commits:
   one labeled e.g. `chore(spec): refresh vendored OpenAPI spec` (carrying
   `docs/frontapp-openapi.yaml`, the regenerated `api/`/`models/`/`client.py`,
   AND the regenerated `docs/api-facts.yaml`), one labeled
