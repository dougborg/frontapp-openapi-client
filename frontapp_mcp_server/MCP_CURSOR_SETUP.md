# Frontapp MCP Server - Cursor IDE Setup

This guide explains how to run the Frontapp MCP server independently and connect Cursor
...
uv run python -m frontapp_mcp --transport sse --port 8765
```
...
- `FRONTAPP_API_KEY` (required): Your Frontapp API key
- `FRONTAPP_BASE_URL` (optional): API base URL (default: `https://api2.frontapp.com`)

Set them in your `.env` file or export them:

```bash
export FRONTAPP_API_KEY=your-api-key-here
export FRONTAPP_BASE_URL=https://api2.frontapp.com  # Optional
```
...
1. Check that `FRONTAPP_API_KEY` is set:

   ```bash
   echo $FRONTAPP_API_KEY
   ```
