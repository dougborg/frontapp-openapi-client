# Structured Logging in Frontapp MCP Server

The Frontapp MCP Server uses structured logging with
...
- **`FRONTAPP_MCP_LOG_LEVEL`** - Log level (default: `INFO`)
  - Options: `DEBUG`, `INFO`, `WARNING`, `ERROR`

- **`FRONTAPP_MCP_LOG_FORMAT`** - Output format (default: `json`)
  - Options: `json`, `text`
...
The Frontapp MCP Server provides convenience decorators to automatically instrument
...
```python
from frontapp_mcp.logging import observe_tool
...
async def search_conversations_impl(
    request: Annotated[SearchConversationsRequest, Unpack()], context: Context
) -> SearchConversationsResponse:
    """Search conversations - automatically instrumented."""
    services = get_services(context)
    return await services.client.conversations.search(request.query)
```
...
```python
from frontapp_mcp.logging import observe_service
