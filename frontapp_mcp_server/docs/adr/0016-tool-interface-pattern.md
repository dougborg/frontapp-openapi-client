...
Date: 2025-01-11 (updated 2026-04-17 for Frontapp fork — examples replaced, core
decision unchanged)
...
Elicitation** for destructive operations. Frontapp's tools are small enough that we use
...
the `@unpack_pydantic_params` decorator (still available via `frontapp_mcp/unpack.py`):
...
Frontapp tools return plain dicts. The mutation tools follow this shape:
...
Common schemas live in `frontapp_mcp/tools/schemas.py` so every mutation tool reuses
