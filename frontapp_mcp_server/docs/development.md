...
Frontapp MCP Server using hot-reload capabilities.
...
uv run mcp-hmr frontapp_mcp.server:mcp
```
...
| `frontapp-mcp-server` | stdio (default) | No         | Production                  |
...
```json
{
  "mcpServers": {
    "frontapp-erp-dev": {
      "command": "/absolute/path/to/frontapp-openapi-client/.venv/bin/mcp-hmr",
      "args": ["frontapp_mcp.server:mcp"],
...
- **Development**: Use `@frontapp-erp-dev` in Claude Desktop
- **Production**: Use `@frontapp-erp` in Claude Desktop
...
#    - Modify src/frontapp_mcp/tools/conversations.py
...
- Tool implementations (`src/frontapp_mcp/tools/*.py`)
- Helper functions
...
```python
# Before (in src/frontapp_mcp/tools/conversations.py)
async def update_conversation(context: Context, conversation_id: str, status: str | None = None, confirm: bool = False) -> dict:
    services = get_services(context)
    if not confirm:
        return {"preview": {"action": "update", "status": status}, "confirmed": False}
    response = await services.client.conversations.update(conversation_id, status=status)
...
async def update_conversation(context: Context, conversation_id: str, status: str | None = None, confirm: bool = False) -> dict:
    ...
    logger.info(f"Updating conversation {conversation_id} to status={status}")

    services = get_services(context)
    if not confirm:
        return {"preview": {"action": "update", "status": status}, "confirmed": False}
    response = await services.client.conversations.update(conversation_id, status=status)
    logger.info(f"Front responded: {response.status_code}")
    return {"confirmed": True, "status_code": response.status_code}
```
...
git commit -m "feat(mcp): add new conversations tool"
```
...
1. Ensure you're running from the correct directory: `cd frontapp_mcp_server`
1. Verify dependencies are installed: `uv sync --extra dev`
1. Check that the file path in the command is correct: `src/frontapp_mcp/server.py:mcp`
...
1. Check that `FRONTAPP_API_KEY` is set in env
...
async def _update_conversation_impl(request, context) -> dict[str, object]:
    \"\"\"Implementation with full business logic.\"\"\"
    # ... implementation

@mcp.tool()
async def update_conversation(request, context) -> dict[str, object]:
    \"\"\"FastMCP tool wrapper - minimal logic here.\"\"\"
    return await _update_conversation_impl(request, context)
```

This allows testing `_update_conversation_impl` directly with mocks.
...
- **Discussions**: https://github.com/dougborg/frontapp-openapi-client/discussions
