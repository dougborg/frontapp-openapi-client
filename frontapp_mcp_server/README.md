...
[Frontapp API](https://api2.frontapp.com). Exposes the API as tools so AI
assistants (Claude Desktop, Claude.ai, Cursor, etc.) can read and update order status
through natural language.

The Frontapp API is intentionally small — seven endpoints for listing and looking up
...
- **Built-in resilience**: automatic retries, 429 rate-limit handling with exponential
  backoff, and auto-pagination inherited from the `frontapp-openapi-client` transport
  layer.
- **Environment-based authentication**: bearer token via `FRONTAPP_API_KEY` (env var,
  `.env`, or `~/.netrc`).
...
### 1. Get your Frontapp API Key

Obtain your API key from your Frontapp account settings.
...
- `frontapp://statuses` — full status catalog (JSON).
...
Your API key is invalid or expired. Rotate it in your Frontapp account settings.
