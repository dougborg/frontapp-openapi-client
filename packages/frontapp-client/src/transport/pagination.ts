/**
 * Auto-pagination transport layer for Frontapp API
 *
 ...
 * - Uses 250 items per page (Frontapp's max) when no limit specified by caller
 ...
 *
 * Mirrors Python client's PaginationTransport pattern from frontapp_client.py
 */

/**
 * Pagination metadata returned from Frontapp API
 */
...
export interface PaginationConfig {
  ...
  maxItems?: number;
  /** Default page size when not specified. Default: 250 (Frontapp API max) */
...
export const DEFAULT_PAGINATION_CONFIG: PaginationConfig = {
  maxPages: 100,
  maxItems: undefined,
  defaultPageSize: 250, // Frontapp API max page size
};
 ...
 * const response = await paginatedFetch('https://api2.frontapp.com/products');
 *
 * // Auto-paginate with custom limit per page
 * const custom = await paginatedFetch('https://api2.frontapp.com/products?limit=100');
 *
 * // Disable auto-pagination with explicit page (ANY page value)
 * const page1 = await paginatedFetch('https://api2.frontapp.com/products?page=1');
 * const page2 = await paginatedFetch('https://api2.frontapp.com/products?page=2');
 * ```
 */
...
async function performPagination(
  ...
  // Get caller's limit or default to 250 (Frontapp's max) for efficiency
