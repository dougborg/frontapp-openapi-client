/**
 * FrontappClient - The main entry point for the Frontapp API client
 *
...
/**
 * Configuration options for FrontappClient
 */
export interface FrontappClientOptions {
  /**
...
/**
 * Default base URL for Frontapp API
 */
...
function resolveApiKey(explicitKey?: string): string {
  ...
  if (typeof process !== 'undefined' && process.env?.FRONTAPP_API_KEY) {
    return process.env.FRONTAPP_API_KEY;
  }

  throw new Error(
    'API key required. Provide via: apiKey option or FRONTAPP_API_KEY environment variable. ' +
...
/**
 * FrontappClient - The main Frontapp API client with automatic resilience
 *
 * Features:
 ...
 * @example Basic usage
 * ```typescript
 * const client = new FrontappClient({ apiKey: 'your-api-key' });
 *
 ...
 * ```typescript
 * const client = new FrontappClient({
 *   apiKey: 'your-api-key',
 *   retry: { maxRetries: 3 },
...
export class FrontappClient {
  ...
  private readonly logger: NonNullable<FrontappClientOptions['logger']>;
  private readonly _sdkClient: Client;

  private constructor(apiKey: string, options: Omit<FrontappClientOptions, 'apiKey'> = {}) {
    this.apiKey = apiKey;
  ...
  /**
   * Create a new FrontappClient instance
   *
   * @param options - Client configuration options
   * @returns Promise resolving to a configured FrontappClient
   *
   * @example
   * ```typescript
   * const client = await FrontappClient.create({ apiKey: 'your-api-key' });
   * ```
   */
  static async create(options: FrontappClientOptions = {}): Promise<FrontappClient> {
    const apiKey = resolveApiKey(options.apiKey);
    return new FrontappClient(apiKey, options);
  }

  /**
   * Create a FrontappClient synchronously with an explicit API key
   *
   * This is a convenience method when you already have the API key.
   * For automatic credential resolution, use `FrontappClient.create()`.
   *
   * @param apiKey - API key for authentication
   * @param options - Additional client options
   * @returns Configured FrontappClient instance
   *
   * @example
   * ```typescript
  ...
  static withApiKey(
    apiKey: string,
    options: Omit<FrontappClientOptions, 'apiKey'> = {}
  ): FrontappClient {
    return new FrontappClient(apiKey, options);
  }

  /**
   * Make an authenticated request to the Frontapp API
   *
   ...
   * @example
   * ```typescript
   * import { listOrders } from 'frontapp-client';
   *
