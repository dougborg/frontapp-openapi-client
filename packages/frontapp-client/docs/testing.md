...
This guide explains the testing architecture and approach for the TypeScript Frontapp
client.
...
**`tests/client.test.ts`** - FrontappClient Tests
...
describe("createPaginatedFetch", () => {
  it("should collect all pages when auto-paginating", async () => {
    ...
    const response = await paginatedFetch("https://api.example.com/conversations");
  ...
  it("should not paginate when explicit page param is present", async () => {
    ...
    const paginatedFetch = createPaginatedFetch(mockFetch);
    await paginatedFetch("https://api.example.com/conversations?page=2");
...
import { FrontappClient } from "../src/client.js";
...
describe("FrontappClient", () => {
  ...
  it("should add Authorization header", async () => {
    ...
    const client = FrontappClient.withApiKey("test-key", {
      fetch: mockFetch,
...
describe("Retry with delays", () => {
  ...
  it("should retry on 429 and succeed", async () => {
    ...
    const responsePromise = client.get("/conversations");
...
describe("create", () => {
  it("should throw descriptive error when no API key", async () => {
    const originalEnv = process.env.FRONTAPP_API_KEY;
    // biome-ignore lint/performance/noDelete: Need to actually remove env var
    delete process.env.FRONTAPP_API_KEY;

    try {
      await expect(FrontappClient.create()).rejects.toThrow(
        /API key required.*apiKey option.*FRONTAPP_API_KEY.*--env-file/,
      );
    } finally {
      if (originalEnv) {
        process.env.FRONTAPP_API_KEY = originalEnv;
      }
    }
  });
  ...
  it("should use API key from environment", async () => {
    const originalEnv = process.env.FRONTAPP_API_KEY;
    process.env.FRONTAPP_API_KEY = "env-api-key";

    try {
      const client = await FrontappClient.create();
      expect(client).toBeInstanceOf(FrontappClient);
    } finally {
      if (originalEnv) {
        process.env.FRONTAPP_API_KEY = originalEnv;
      } else {
...
Integration tests require a real API key and hit the actual Frontapp API.
...
describe("Integration Tests", () => {
  ...
  runIntegration("should fetch orders from real API", async () => {
    const client = FrontappClient.withApiKey(apiKey!, {
  ...
  runIntegration("should handle rate limiting", async () => {
    ...
    const responses = await Promise.all([
      client.get("/conversations", { per_page: 1 }),
      client.get("/statuses"),
...
const client = FrontappClient.withApiKey("key", { fetch: mockFetch });
