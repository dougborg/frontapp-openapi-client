...
frontapp-openapi-client project. Follow these guidelines to work efficiently and safely
...
```
@agent-dev implement the search_conversations tool following the list_conversations pattern
```
...
1. **Implementation Phase**
   ...
   ```
   @agent-dev implement a new tool for filtering by date range, following the list_conversations pattern
...
- `client` - Releases frontapp-openapi-client
- `mcp` - Releases frontapp-mcp-server
- (no scope) - Releases frontapp-openapi-client (default)
...
git worktree add ../frontapp-feature-123 -b feature/123-my-task

# Work in the worktree (each Claude Code session gets its own)
cd ../frontapp-feature-123

# When done, clean up
git worktree remove ../frontapp-feature-123
```
