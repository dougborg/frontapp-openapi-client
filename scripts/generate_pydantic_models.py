...
# Domain groupings for Frontapp. The spec is small (18 schemas) so only two
# topical groups plus a shared "errors" group are needed.
...
def add_base_entity_extra_ignore(classes: list[ClassInfo]) -> list[ClassInfo]:
    ...
    from the Frontapp API, while request models keep extra='forbid' from the base.
    See: https://github.com/dougborg/frontapp-openapi-client/issues/295
    """
    fixed_classes = []
    for cls in classes:
...
def generate_module_imports(
    ...
    for imp in imports:
        ...
        if imp.is_from_import and imp.module and "FrontappPydanticBase" in imp.names:
            continue
    ...
    import_lines.insert(
        0,
        "from frontapp_public_api_client.models_pydantic._base import FrontappPydanticBase",
    )
...
The models in this package mirror the attrs models in frontapp_public_api_client/models/
...
def generate_auto_registry(
    ...
    for py_file in attrs_models_dir.glob("*.py"):
        ...
        for match in re.finditer(class_pattern, content):
            ...
            module_path = f"frontapp_public_api_client.models.{py_file.stem}"
...
def format_code(workspace_path: Path) -> None:
    ...
    pydantic_dir = workspace_path / "frontapp_public_api_client" / "models_pydantic"
...
def main() -> None:
    ...
    output_dir = (
        workspace_path / "frontapp_public_api_client" / "models_pydantic" / "_generated"
    )
    attrs_models_dir = workspace_path / "frontapp_public_api_client" / "models"
    auto_registry_path = (
        workspace_path
