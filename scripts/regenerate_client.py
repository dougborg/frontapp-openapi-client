#!/usr/bin/env python3
"""
Regenerate the Frontapp OpenAPI client from the specification.
...
def validate_openapi_spec(spec_path: Path) -> bool:
    ...
    # Run Redocly validation (advisory only — we vendor Front's spec and can't
    # edit upstream, so Redocly warnings don't block regeneration).
    _validate_with_redocly(spec_path)
...
def _fix_types_imports(target_client_path: Path) -> None:
    ...
    for py_file in target_client_path.rglob("*.py"):
        if py_file.name in ["__init__.py", "frontapp_client.py", "log_setup.py"]:
        ...
        try:
            ...
            patterns = [
                ...
                # Absolute imports
                (
                    r"from frontapp_public_api_client\.types import",
...
def move_client_to_workspace(workspace_path: Path) -> bool:
    ...
    temp_client_dir = workspace_path / "openapi_gen_temp"
    source_client_path = temp_client_dir / "frontapp_public_api_client"
    target_client_path = workspace_path / "frontapp_public_api_client"
    ...
    try:
        ...
        init_content = '''"""Frontapp Public API Client — Python client for the Frontapp API."""
...
def post_process_generated_docstrings(workspace_path: Path) -> bool:
    ...
    """
    client_path = workspace_path / "frontapp_public_api_client"
    if not client_path.exists():
        return True

    # Only process generated files (not custom files like frontapp_client.py, log_setup.py)
    generated_patterns = [
        "api/**/*.py",
        "models/**/*.py",
        "client.py",
...
def fix_specific_generated_issues(workspace_path: Path) -> bool:
    ...
    client_types_file = (
        workspace_path / "frontapp_public_api_client" / "client_types.py"
    )
...
def fix_pagination_defaults(workspace_path: Path) -> None:
    ...
    which affects FrontappClient's automatic pagination feature.
...
def main():
    ...
    spec_path = workspace_path / "docs" / "frontapp-openapi.yaml"
    client_path = workspace_path / "frontapp_public_api_client"

    print("🚀 Frontapp OpenAPI Client Regeneration")
    print("=" * 50)
    ...
    print("\n💡 Next steps:")
    print("   1. Review the generated client in ./frontapp_public_api_client/")
