...
Sanitization:
    ...
    3. Strips inherited defaults that break allOf merges (see
       ``PROPERTY_DEFAULTS_TO_STRIP``).
    4. Replaces Unicode confusables in string values. Front's spec uses
       U+2139 INFORMATION SOURCE in several message-endpoint descriptions;
       when openapi-python-client copies these into docstrings, ruff's RUF002
       flags them as ambiguous (visually indistinguishable from ASCII `i`).
       Replacing at vendor time keeps generated code clean.
...
# Unicode confusables to replace in string values. ruff's RUF002 flags these
# when they appear in generated Python docstrings because they're visually
# indistinguishable from ASCII equivalents but parse as different characters.
# Replace at vendor time so generated code stays clean without per-file
# ignores. Extend this map when regeneration surfaces new confusables.
# Built with chr() so this source file itself doesn't contain the confusables
# it's sanitizing — otherwise ruff's RUF001 would flag the dict keys.
CONFUSABLE_REPLACEMENTS: dict[str, str] = {
    chr(0x2139): "i",  # U+2139 INFORMATION SOURCE → Latin small letter i
}
...
def _replace_confusables(node: object) -> object:
    """Walk the spec tree, replacing Unicode confusables in every string.

    Returns the transformed tree; mutates in place for dicts/lists and
    returns replacement scalars for strings (since strings are immutable).
    """
    if isinstance(node, dict):
        for k, v in node.items():
            node[k] = _replace_confusables(v)
        return node
    if isinstance(node, list):
        for i, v in enumerate(node):
            node[i] = _replace_confusables(v)
        return node
    if isinstance(node, str):
        for confusable, replacement in CONFUSABLE_REPLACEMENTS.items():
            if confusable in node:
                node = node.replace(confusable, replacement)
        return node
    return node
