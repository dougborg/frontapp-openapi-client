#!/usr/bin/env python3
"""Build the Frontapp MCP server as an MCP Bundle (``.mcpb``).
...
1. Read the package version from ``frontapp_mcp_server/pyproject.toml``.
2. Stage a self-contained bundle directory under ``build/mcpb/`` containing:

   - ``manifest.json``           — derived from
     ``frontapp_mcp_server/mcpb/manifest.template.json`` with ``__VERSION__``
     substituted.
   - ``pyproject.toml``          — derived from
     ``frontapp_mcp_server/mcpb/pyproject.template.toml`` with ``__VERSION__``
   ...
   - ``src/frontapp_mcp/``       — copied from
     ``frontapp_mcp_server/src/frontapp_mcp/``.
   - ``README.md``               — copied from the package's own README.

3. Validate ``manifest.json`` against the MCPB schema via ``mcpb validate``.
4. Run ``mcpb pack build/mcpb dist/frontapp-mcp-server-<version>.mcpb`` to
...
PKG_ROOT = REPO_ROOT / "frontapp_mcp_server"
PKG_PYPROJECT = PKG_ROOT / "pyproject.toml"
PKG_SRC = PKG_ROOT / "src" / "frontapp_mcp"
...
def stage_bundle(version: str) -> None:
    ...
    src_dest = BUILD_DIR / "src" / "frontapp_mcp"
    shutil.copytree(
        PKG_SRC,
        src_dest,
...
def run_mcpb_pack(version: str) -> Path:
    ...
    artifact = DIST_DIR / f"frontapp-mcp-server-{version}.mcpb"
    if artifact.exists():
        artifact.unlink()
    subprocess.run(
