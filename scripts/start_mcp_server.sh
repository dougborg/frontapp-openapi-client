#!/usr/bin/env bash
# Start Frontapp MCP Server with SSE transport for independent operation
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --transport)
      ...
      ;;
    -h|--help)
      echo "Start Frontapp MCP Server with SSE transport"
      echo ""
      ...
      echo "  FRONTAPP_API_KEY   Required: Your Frontapp API key"
      echo "  FRONTAPP_BASE_URL  Optional: API base URL (default: https://api2.frontapp.com)"
...
uv run python -m frontapp_mcp --transport "$TRANSPORT" --host "$HOST" --port "$PORT"
