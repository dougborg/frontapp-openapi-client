"""Test configuration and fixtures for the Frontapp OpenAPI Client test suite."""
...
def mock_api_credentials():
    ...
    return {
        ...
        "base_url": "https://api.frontapp.test",  # .test TLD reserved for testing (RFC 6761)
    }


@pytest.fixture
def frontapp_client(mock_api_credentials):
    """Create a FrontappClient for testing."""
...
def setup_test_env(monkeypatch, request):
    ...
    if is_real_api_test:
        ...
        if not os.getenv("FRONTAPP_API_KEY"):
            monkeypatch.setenv("FRONTAPP_API_KEY", "test-key")
        if not os.getenv("FRONTAPP_BASE_URL"):
            monkeypatch.setenv("FRONTAPP_BASE_URL", "https://api2.frontapp.com")
    else:
        # For unit tests, use test values with .test TLD (RFC 6761 reserved for testing)
        monkeypatch.setenv("FRONTAPP_API_KEY", "test-key")
        monkeypatch.setenv("FRONTAPP_BASE_URL", "https://api.frontapp.test")
