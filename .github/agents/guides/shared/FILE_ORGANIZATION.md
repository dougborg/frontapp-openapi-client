...
(`docs/frontapp-openapi.yaml`). **Any manual edits will be overwritten** during
regeneration.
...
# frontapp_public_api_client/api/conversations/list_conversations.py
...
```python
# DO NOT EDIT THIS FILE
# frontapp_public_api_client/models/product.py
...
**Do not modify** - Use `FrontappClient` instead (see Editable Files section).
...
- `FrontappClient` - Wrapper around `AuthenticatedClient`
...
│   ├── frontapp_client.py                  # ✅ EDITABLE - Custom client
...
│   ├── frontapp-openapi.yaml               # ✅ EDITABLE - OpenAPI spec
│   ├── adr/                              # Architecture decisions
│   └── *.md                              # User guides
│
├── frontapp_mcp_server/                    # ✅ EDITABLE - MCP server package
│   └── ... (separate package)
│
...
Validates `docs/frontapp-openapi.yaml` with:
...
1. Moves generated code to `frontapp_public_api_client/`
...
- Preserves editable files (`frontapp_client.py`, `log_setup.py`)
...
from frontapp_public_api_client.api.sales_order import create_sales_order
```

**For generated models:**

```python
from frontapp_public_api_client.models import Product, SalesOrder
```
...
from frontapp_public_api_client.client_types import Response, UNSET
```
...
from frontapp_public_api_client.generated.api.conversations import list_conversations

# DON'T - Use client_types instead
from frontapp_public_api_client.types import Response

# DON'T - Use FrontappClient instead
from frontapp_public_api_client.client import AuthenticatedClient
```
...
```python
# Editing frontapp_public_api_client/api/conversations/list_conversations.py
def list_conversations(client, limit=100):  # Manual change
    # ... modified code ...
```

**Solution:** Extend functionality in `frontapp_client.py` or create wrapper functions
in editable files.
...
1. Update the OpenAPI spec (`docs/frontapp-openapi.yaml`)
1. Add custom logic in `frontapp_client.py`
...
```python
from frontapp_public_api_client.types import Response  # Conflicts with stdlib
```

**Solution:**

```python
from frontapp_public_api_client.client_types import Response  # Correct
```
...
async with FrontappClient() as client:
    ...
    await list_conversations.asyncio_detailed(client=client)
...
- Custom behavior in `frontapp_client.py` only
...
- ✅ **Always use** `frontapp_client.py` for custom client logic
- 🔄 **Regenerate with** `uv run poe regenerate-client` (2+ min)
- 📦 **Import from** `client_types` not `types`
- 🎯 **Use** `FrontappClient` not base `AuthenticatedClient`
