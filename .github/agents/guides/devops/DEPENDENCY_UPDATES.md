...
```bash
# Add to frontapp-openapi-client
cd frontapp_public_api_client
uv add httpx

# Add to frontapp-mcp-server
...
```toml
[tool.uv.workspace]
members = ["frontapp_public_api_client", "frontapp_mcp_server"]
```

### Package Dependencies

**frontapp-openapi-client** (independent):

- httpx, attrs, python-dateutil, etc.
- No dependency on MCP server

**frontapp-mcp-server** (depends on client):

```toml
[project]
dependencies = [
    "frontapp-openapi-client",  # Internal dependency
    "mcp>=1.0.0",
    # ...
]
```
