...
Error: Files would be reformatted:
  frontapp_mcp_server/src/frontapp_mcp/tools/inventory.py
```
...
```
frontapp_mcp_server/src/frontapp_mcp/tools/inventory.py:42:5: F841 Local variable `result` is assigned to but never used
```
...
```
frontapp_mcp_server/src/frontapp_mcp/tools/inventory.py:42: error: Incompatible return value type (got "str", expected "int")
```
...
```
WARNING - Could not find 'frontapp_mcp.tools.inventory' in docstrings
```
