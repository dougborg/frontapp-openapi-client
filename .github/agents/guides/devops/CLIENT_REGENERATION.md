...
- New endpoints added to `docs/frontapp-openapi.yaml`
...
git log --oneline docs/frontapp-openapi.yaml

# Check generated code timestamp
ls -l frontapp_public_api_client/api/
```
...
- Fix errors in `docs/frontapp-openapi.yaml`
...
```bash
# Create backup
cp -r frontapp_public_api_client frontapp_public_api_client.backup

# Or commit current state
git add frontapp_public_api_client
...
git diff frontapp_public_api_client/api/

# Focus on model changes
git diff frontapp_public_api_client/models/
```
...
frontapp_public_api_client/py.typed
```
...
```
frontapp_public_api_client/frontapp_client.py  ✅ Preserved
frontapp_public_api_client/log_setup.py      ✅ Preserved
```
...
# Update custom code (frontapp_client.py) if needed
```
...
1. Check what changed: `git diff frontapp_public_api_client/models/`
...
**File:** `docs/frontapp-openapi.yaml`
...
```yaml
packageName: frontapp_public_api_client
projectName: frontapp-openapi-client
...
rm -rf frontapp_public_api_client
mv frontapp_public_api_client.backup frontapp_public_api_client

# Option 2: Git revert
git restore frontapp_public_api_client/
...
**Remember:** Generated code is read-only. Customizations go in `frontapp_client.py`!
