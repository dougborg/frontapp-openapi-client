...
## description: 'Instructions for building Model Context Protocol (MCP) servers using the Python SDK with FastMCP' applyTo: '**/frontapp_mcp_server/**/\*.py'
...
## Frontapp MCP Server Patterns

### ServerContext Pattern (ADR-010)

- Use `get_services()` to access FrontappClient and other services
- Never instantiate FrontappClient directly in tools
...
async def check_inventory(sku: str) -> str:
    ...
    # Use services.frontapp_client for API calls
...
async def risky_operation(input: str) -> str:
    ...
    try:
        services = get_services()
        result = await perform_operation(services.frontapp_client, input)
