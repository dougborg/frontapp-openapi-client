...
- **frontapp-openapi-client**: `0.23.0` (tracks main client)
- **frontapp-mcp-server**: `0.1.0a1` (alpha release)
...
- **frontapp-openapi-client**: Publishes from `release-client` job
- **frontapp-mcp-server**: Publishes from `release-mcp` job
...
1. **Update**: Updates `frontapp-openapi-client>=X.Y.Z` in
   `frontapp_mcp_server/pyproject.toml`
...
- **MCP Server**: `frontapp_mcp_server/CHANGELOG.md`
...
```bash
uv run semantic-release version --print --no-commit --no-tag --no-push \
  -c frontapp_mcp_server/pyproject.toml
```
...
1. Updates the `frontapp-openapi-client>=X.Y.Z` dependency in
   `frontapp_mcp_server/pyproject.toml`
...
1. Manually update `frontapp_mcp_server/pyproject.toml`
