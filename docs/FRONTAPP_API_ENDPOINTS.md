...
This document provides a comprehensive list of all available Frontapp API endpoints that
can be incorporated into the MCP Server.

**Last Updated**: 2025-10-21 **API Base URL**: `https://api2.frontapp.com`
...
**Use Cases**: Generate detailed reports on team performance, response times, message
volumes

**Note**: Current implementation only has basic analytics endpoint. Need to add
export/report creation and retrieval.
...
**Use Cases**: Manage communication channels (email, SMS, social media), sync messages
...
**Note**: API only supports listing fields and updating values (no field creation via
API)
...
**Important**: Editing/deleting drafts requires a `version` field to prevent conflicts
...
**Special Feature**: Event IDs are sequential and chronologically ordered (base 36 after
`evt_` prefix)
...
**Use Cases**: Connect Front conversations with external systems (CRM, ticketing,
project management)
...
**Special Note**: Creating a message returns a `message_uid` (202 response), not the
final `message_id`
...
**Definition**: Shifts are recurring time intervals assigned to teammates; Front
automatically manages availability status when shifts start/end
...
4. **Phase 4** (Specialized features): Knowledge Bases, Shifts, Signatures, Analytics
   Reports
