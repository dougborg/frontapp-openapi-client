# uv Package Manager Guide for Frontapp OpenAPI Client

This document provides common uv commands for working with the Frontapp OpenAPI Client
project.
...
See [ADR-009](adr/0009-migrate-from-poetry-to-uv.md) for the full migration rationale.
...
# - frontapp-openapi-client (main package, current directory)
# - frontapp-mcp-server (MCP server package, separate directory)
```
...
```toml
[tool.uv.workspace]
members = [".", "frontapp_mcp_server"]
```

This allows both packages to share a unified lock file (`uv.lock`) and ensures version
compatibility. See
[ADR-009: Use uv as the Package Manager](adr/0009-migrate-from-poetry-to-uv.md) for the
package-manager choice and
[ADR-013: Module-Local Documentation](adr/0013-module-local-documentation.md) for the
monorepo layout.
...
uv run pytest tests/test_frontapp_client.py  # Specific file
...
# Quick import test
uv run python -c "from frontapp_public_api_client import FrontappClient; print('✅ Import successful')"
```
...
# Run a specific test
uv run pytest tests/test_frontapp_client.py::TestFrontappClient::test_client_initialization -v

# Check test coverage for specific module
uv run pytest tests/ --cov=frontapp_public_api_client.frontapp_client --cov-report=term-missing
```
...
- [ADR-009: Use uv as the Package Manager](adr/0009-migrate-from-poetry-to-uv.md)
