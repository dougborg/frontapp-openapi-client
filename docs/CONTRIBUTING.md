# Contributing to Frontapp OpenAPI Client

Thank you for your interest in contributing to the Frontapp OpenAPI Client! This
...
1. **Set up environment variables**
   ...
   ```bash
   cp .env.example .env
   # Edit .env with your Frontapp API credentials for testing
   ```
...
uv run poe test tests/test_frontapp_client.py
...
- Files in `frontapp_public_api_client/` → use `(client)` scope
- Files in `frontapp_mcp_server/` → use `(mcp)` scope
...
├── frontapp_public_api_client/    # Main package
│   ├── __init__.py
│   ├── frontapp_client.py         # Main client implementation
...
Integration tests require valid Frontapp API credentials. Set these in your `.env` file:
...
The OpenAPI client is automatically generated from `frontapp-openapi.yaml` using
...
- `frontapp_client.py` - Our resilient client implementation
